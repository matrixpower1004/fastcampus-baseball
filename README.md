...
* 마지막으로는 enum을 통한 method 주입을 구현해 볼 수는 없을까라는 생각에 enum을 통한 컨트롤러를 구현하게 되었습니다.
...
* 팀장 : 이지상
    ...
    * Java 14부터 지원된 record 클래스를 활용해 보고자 Java 17을 선택했는데 builder pattern에 길들여져서(?) record를 활용해 보지 못한 점이 아쉬었습니다. 다른 프로젝트엔 꼭~! (Really?)
    * GitFlow brach 전략을 사용하지 못한 점이 아쉬었습니다. 프로젝트의 커밋이 처음 생각보다 많아지면서 Devlop branch의 필요성이 대두되었고, 다음 프로젝트에는 꼭 Develop branch를 이용하는 GitFlow branch 전략을 적용해 보고 싶네요.
    * Functional interface를 비롯하여 지금까지 배우고 익힌 것들을 마음껏 활용해 본 프로젝트였습니다.
    * 추상화를 어느 수준까지 구현할 수 있는지 스스로에 대한 도전적인 성격의 프로젝트였고, 다음 프로젝트에서는 역할과 책임의 관점에서 조금 더 수준 높은 추상화에 도전해 볼 수 있는 자신감을 준 프로젝트였습니다.
      
* 팀원 : 김용원
    * 객체지향의 책임과 역할에 대해서 자세히 알게되는 시간이었습니다.
    * Spring을 하기전에 java 프로그램에대해 복습할 수 있던 좋은 시간이었습니다.
    * 협업을 처음 해서 제가 자주 실수한 부분에 대해 고쳐야 할 점과 생각하지 못했던 부분에 대해서 많은 것을 배울 수 있는 시간이 되었습니다.
    * Github에서 Project 이슈관리를 어떻게 하며 커밋 메세지를 어떻게 사용하는지 알게되었습니다.
